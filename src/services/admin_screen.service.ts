import { BulkCampaignStatus, DeviceStatus } from "@prisma/client";
...
export const ADMIN_SCREEN_MODULE_IDS = [
  "feature-flags",
  "integrations",
  "system",
  "audit",
...
export type AdminScreenModuleId = (typeof ADMIN_SCREEN_MODULE_IDS)[number];
...
export type AdminScreenJson = {
  moduleId: string;
  scope: "platform" | "workspace";
  generatedAt: string;
  meta: AdminScreenMeta;
  kpis: AdminScreenKpi[];
  tables: AdminScreenTable[];
  notes: string[];
};

function isScreenModuleId(s: string): s is AdminScreenModuleId {
  return (ADMIN_SCREEN_MODULE_IDS as readonly string[]).includes(s);
}

export async function getAdminScreen(
  moduleId: string,
  operatorEmail: string,
  workspaceId: string
): Promise<AdminScreenJson> {
  if (!isScreenModuleId(moduleId)) {
    throw new AppError(404, "Unknown admin screen", "NOT_FOUND");
  }
  ...
  const notes: string[] = [];
  let kpis: AdminScreenKpi[] = [];
  let tables: AdminScreenTable[] = [];
  let noData = false;
  let message: string | null = null;

  switch (moduleId) {
    case "feature-flags": {
      const hasOperatorList = Boolean(env.PLATFORM_OPERATOR_EMAILS?.trim());
      kpis = [
        {
          label: "Stripe",
          value: env.STRIPE_SECRET_KEY ? "Configured" : "Off",
          hint: "Billing integration",
        },
        {
          label: "WhatsApp bridge",
          value: env.WHATSAPP_BRIDGE_ENABLED ? "On" : "Off",
          hint: "Real sends / QR",
        },
        {
      ...
      notes.push(
        "There is no dedicated feature-flag store yet; toggles follow environment and workspace billing state."
      );
      noData = false;
      message = null;
      break;
    ...
    case "integrations": {
      const [webhook24h, webhookTotal] = await Promise.all([
        prisma.stripeEventLog.count({ where: { processedAt: { gte: h24 } } }),
        prisma.stripeEventLog.count(),
      ]);
      kpis = [
        {
          label: "Stripe API",
          value: env.STRIPE_SECRET_KEY ? "Secret configured" : "Not set",
          hint: env.STRIPE_SECRET_KEY ? "Live or test key present" : undefined,
        },
        {
          label: "Webhooks (24h)",
      ...
      if (platform) {
        ...
        message = null;
        notes.push(
          "Full webhook streams are listed in platform scope (operator email in PLATFORM_OPERATOR_EMAILS)."
        );
      }
      break;
    }
    ...
    case "system": {
      ...
      kpis = [
        ...
        {
          label: "App URL",
          value: env.APP_PUBLIC_URL.replace(/^https?:\/\//, "").slice(0, 40),
          hint: "APP_PUBLIC_URL",
        },
        {
          label: "Workspaces",
      ...
      tables.push({
        ...
        rows: [
          { key: "WhatsApp bridge", value: env.WHATSAPP_BRIDGE_ENABLED ? "enabled" : "disabled" },
          { key: "Stripe", value: env.STRIPE_SECRET_KEY ? "configured" : "not configured" },
          { key: "JSON body limit", value: env.HTTP_JSON_BODY_LIMIT },
        ],
      });
      noData = false;
      message = null;
    ...
    case "audit": {
      if (platform) {
        ...
        tables.push({
          ...
          description:
            "A dedicated immutable audit trail is not in the database yet; this is a stand-in timeline.",
          columns: [
            { field: "type", header: "Type" },
            { field: "eventId", header: "Event ID" },
    ...
    case "support": {
      ...
      const [threadCount, msgs24h, active7d, threads] = await Promise.all([
        prisma.liveChatThread.count({ where: threadWhere }),
        ...
        prisma.liveChatThread.findMany({
          where: threadWhere,
          select: {
            peerPhone: true,
            peerLabel: true,
      ...
      tables.push({
        ...
        rows: threads.map((t) => {
          ...
          const lastAt =
            t.lastMessageAt?.toISOString() ?? t.updatedAt.toISOString();
    ...
    case "reports": {
      ...
      const [contacts, templates, outbound30d, bulkDone] = await Promise.all([
        ...
        prisma.messageTemplate.count({ where: w ? { workspaceId } : {} }),
...
function threadWhereToOutbound(w: { workspaceId: string } | undefined) {
