...
exports.sanitizePhoneRawInput = sanitizePhoneRawInput;
exports.validateAndFormatPhone = validateAndFormatPhone;
exports.validatePhoneForGrabberImport = validatePhoneForGrabberImport;
 ...
 */
function sanitizePhoneRawInput(raw) {
    let s = raw.normalize("NFC").trim().replace(ZERO_WIDTH, "");
    const waMe = s.match(/(?:https?:\/\/)?(?:www\.)?wa\.me\/\+?(\d{6,18})(?:[/?#]|$)/i);
    if (waMe) {
        s = `+${waMe[1]}`;
    }
...
function defaultCountry() {
    const r = env_1.env.PHONE_DEFAULT_REGION;
    return r && /^[A-Z]{2}$/.test(r) ? r : undefined;
}
const LID_MESSAGE = "WhatsApp export shows an internal ID (…@lid), not a phone number. Use a column with …@s.whatsapp.net, plain digits with country code, or set PHONE_DEFAULT_REGION for local numbers.";
const GUS_MESSAGE = "This value is a WhatsApp group id (@g.us), not a contact phone.";
const GENERIC_INVALID = "Use a valid international number (e.g. +880 1XXX-XXXXXX) or enable PHONE_DEFAULT_REGION for local numbers without country code.";
function splitIntoPhoneCandidates(trimmed) {
...
function validateAndFormatPhoneSegment(segment) {
    ...
    let parsed = (0, max_1.parsePhoneNumberFromString)(prepared, dc);
    if (!parsed?.isValid() && dc) {
        parsed = (0, max_1.parsePhoneNumberFromString)(prepared);
    }
    if (!parsed?.isValid()) {
        const d = digitsOnly(prepared);
        if (d.length >= 8 && d.length <= 15) {
            parsed = (0, max_1.parsePhoneNumberFromString)(`+${d}`);
        }
    }
...
function validateAndFormatPhone(raw) {
 ...
 */
function validatePhoneForGrabberImport(raw) {
    ...
    if (d.length < 8 || d.length > 15) {
        return {
            valid: false,
            e164: null,
            message: "Phone must be 8–15 digits in international form (after + country code).",
        };
    }
    if (d.startsWith("0")) {
        return {
            valid: false,
            e164: null,
            message: "International numbers cannot start with 0; include country code (e.g. +880…).",
        };
    }
